...
*/
Node* interleave(Node* list1, Node* list2){
  Node *merged = NULL, *cursor = NULL;
  if (!list1 && !list2) return merged;
  if (list1) {
    merged = new Node;
    merged->key = list1->key;
  }
  if (list2) {
    if (merged) {
      merged->next = cursor = new Node;
      cursor->key = list2->key;
      cursor->next = interleave(list1->next, list2->next);
    }
    else {
      merged = new Node;
      merged->key = list2->key;
      merged->next = interleave(NULL, list2->next);
    }
  } else {
    // since list2 is empty, list1 should not be empty 
    merged->next = interleave(list1->next, NULL);
  }
  return merged;
}
