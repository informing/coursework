...
void balance(Node *&root) {
	...
	int maxk = root->key, midk = 0, mink = 0;
	Node *left = root->left, *right = root->right;
	if (left == NULL && right == NULL) return;
	if (left == NULL) {
		if (right->key >= maxk) return;
		else std::swap(root->key, right->key);
	}
	if (right == NULL) {
		if (left->key < maxk) return;
		else std::swap(root->key, left->key);
	}
	if (left->key < maxk && right->key >= maxk) return;
	if (maxk <= left->key) mink = midk, midk = maxk, maxk = left->key;
	else midk = left->key;
	if (maxk <= right->key) mink = midk, midk = maxk, maxk = right->key;
	else mink = right->key;
	if (mink > midk) std::swap(mink, midk);
	root->key = midk;
	left->key = mink;
	right->key = maxk;
}
...
bool delete_node(Node*& root, int key) {
  ...
  // case 3: target has two children
     ...
     */
    Node *pred = parent, *prev = NULL;
    bool right = false;
    if (target != NULL) {
      prev = pred;
      pred = target->left;
    }
    std::cout<<"before:\n"<<std::endl;printTree(root, 0);
    ...
    std::cout << "key " << target->key << " with pred:";
    assert (pred != NULL && target != NULL);
    if (parent != NULL) {
      std::cout << pred->key <<std::endl;
...
int depth( Node* root, Node* x ) {
  /**
   * Q6: Needs to be changed ********************
   */
  if (x == root) return 0; // comment out this line
  if (x->key < root->key)
