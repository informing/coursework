...
//uniform color ambientColorUniform;
...
void main() {
	...
	float diffuse = clamp(dot(normal, lightDirection.xyz), 0.0, 1.0);
	...
	if (dot(normal, lightDirection) < 0.0) {
		...
		light_SPC = vSpecular * vLightColor * pow(clamp(dot(bounce, view), 0.0, 1.0), vShininess);
	}
